...
export default function Library() {
  ...
  const { data: pastEvents, isLoading: pastLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("events")
        .select("id, title, date_time, end_date_time, location, address, status, cover_photo_url, event_type_id, capacity, has_potluck, virtual_link, zoom_link, online_link, is_hybrid, host_id, description, venue_id, ticket_fee, mureeds_only, age_group, location_hint, etiquette_notes, payment_instructions, waitlist_capacity, created_at, updated_at")
